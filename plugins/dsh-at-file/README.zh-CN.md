...
- **键盘导航**：菜单里高亮是**目录**时按 **Enter** 就**进入该目录**（菜单立即刷新为它的直接子项，如 `@目录/`）；按 **Tab** 则**直接确定选择**——**不管是文件还是文件夹**都插入 `@路径 ` 收尾（目录不会进入下级）。文件 + Enter 走正常选择流程。鼠标点击直接选中（插入 `@路径 `）。
...
键盘导航：高亮是**目录**时按 **Enter** 进入该目录（菜单刷新为它的子项，可一路深入，如 `@plugins/dsh-at-file/lib/`）；按 **Tab** 则**直接确定选择**——不管是文件还是文件夹都插入 `@路径 `，目录不进入下级。鼠标点击直接选中当前条目。
