// dsh-free-vision 产物门检查（免启动）：验证包自洽，可在 CI/本地跑。
...
// 0. 语法检查（不需要依赖解析）。
for (const f of ['src/index.js', 'src/swift.js', 'src/upload.js', 'lib/client.js']) {
...
// 1.5 本地识别脚本存在（macOS Vision 业务逻辑所在）。
if (!existsSync(resolve(root, 'scripts/ocr.swift'))) fail('scripts/ocr.swift 不存在')
else ok('scripts/ocr.swift 存在')
...
if (!patchRow) fail('cordis.patch.yml 里找不到 insert 行')
  ...
  if (rowId !== 'free-vision') fail(`patch 行 id (${rowId}) 应为 free-vision（插件名）`)
...
if (schemasteryResolvable) {
  ...
  if (host.name !== 'free-vision') fail(`host.name (${host.name}) 应为 free-vision（插件名，非包名）`)
  else ok('host.name = free-vision（与 patch 行 id 一致）')
...
// 3.5 子模块导出形状（swift.js / upload.js）。
const swiftSrc = readFileSync(resolve(root, 'src/swift.js'), 'utf8')
for (const pat of [/export const DEFAULT_SCRIPT/, /export async function prepareImage\(/, /export async function runSwift\(/, /export function sniffImageExt\(/, /export function isBlockedDownloadUrl\(/]) {
  if (pat.test(swiftSrc)) ok(`swift.js 含 ${pat}`)
  else fail(`swift.js 缺少 ${pat}`)
}
const uploadSrc = readFileSync(resolve(root, 'src/upload.js'), 'utf8')
for (const pat of [/export const UPLOAD_PATH/, /export function registerUploadRoute\(/, /export function createUploadHandler\(/, /export function isLoopback\(/]) {
  if (pat.test(uploadSrc)) ok(`upload.js 含 ${pat}`)
  else fail(`upload.js 缺少 ${pat}`)
}
...
if (!clientSrc.includes('conversation.input.right')) fail('client bundle 未注册 conversation.input.right 槽')
else ok('client bundle 注册 conversation.input.right 槽（粘贴→路径）')
if (!clientSrc.includes('/plugins/dsh-free-vision/images')) fail('client bundle 上传路径与 host 不一致')
else ok('client bundle 上传路径 /plugins/dsh-free-vision/images 与 host 一致')
